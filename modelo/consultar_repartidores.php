<?php
    try{
        ...
        foreach( $preparada->fetchAll(PDO::FETCH_ASSOC) as $repartidor ){
            ...
            $preparada = $conexion->prepare("SELECT id, latitud, longitud, velocidad, DATEDIFF(SECOND, '1970-01-01 00:00:00.000', fecha) * 1000 AS fecha FROM posiciones WHERE usuario = :repartidor AND fecha >= :dia_inicial AND fecha < DATEADD(DAY, 1, :dia_final)");
