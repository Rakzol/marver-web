...
<body>
    ...
    <h1 class="text-center" >Preventas</h1>
    ...
    <script>
        ...
        let ultima;
        let pagina;
        let preventas_pagina = 9;
        let preventas = [];
        let cuerpo_preventas;

        window.addEventListener('load', () => {
            primera = document.getElementById('primera');
            ultima = document.getElementById('ultima');
            pagina = document.getElementById('pagina');
            cuerpo_preventas = document.querySelector('tbody');

            fetch('modelo/consultar_preventas')
                .then((respuesta) => {
                    return respuesta.json();
                })
                .catch(error => {
                    console.error('Error al solicitar los preventas: ', error);
                })
                .then(respuesta_json => {
                    preventas = respuesta_json;
                    //console.log(preventas);
                    primera.innerText = preventas.length ? 1 : 0;
                    pagina.valueAsNumber = preventas.length ? 1 : 0;
                    ultima.innerText = Math.ceil(preventas.length / preventas_pagina);
                    mostrar_preventas();
                });
        });
        ...
        function mostrar_preventas() {
            cuerpo_preventas.replaceChildren();
            let inicio = (pagina.valueAsNumber - 1) * preventas_pagina;
            preventas.slice(inicio, inicio + preventas_pagina).forEach(pedido => {
                let tr = document.createElement('tr');

                let td = document.createElement('td');
                td.innerText = pedido['Folio'];
                tr.appendChild(td);
                ...
                cuerpo_preventas.appendChild(tr);
            });
        }
