...
<body>
    ...
    <script>
        ...
        window.addEventListener('load', () => {
            ...
            cuerpo_pedidos = document.querySelector('tbody');

            fetch('modelo/consultar_pedidos')
                .then((respuesta) => {
                    return respuesta.json();
                })
                .catch(error => {
                    console.error('Error al solicitar los pedidos: ', error);
                })
                .then(respuesta_json => {
                    pedidos = respuesta_json;
                    //console.log(pedidos);
                    primera.innerText = pedidos.length ? 1 : 0;
                    pagina.valueAsNumber = pedidos.length ? 1 : 0;
                    ultima.innerText = Math.ceil(pedidos.length / pedidos_pagina);
                    mostrar_pedidos();
                });
        });
        ...
        function mostrar_pedidos() {
            cuerpo_pedidos.replaceChildren();
            let inicio = (pagina.valueAsNumber - 1) * pedidos_pagina;
            pedidos.slice(inicio, inicio + pedidos_pagina).forEach(pedido => {
